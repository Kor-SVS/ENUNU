...
def pip_install_upgrade(python_exe: str, packages: List[str]):
    ...
    pythonのパッケージを更新する
    """
    args = [python_exe, "-m", "pip", "install", "--upgrade", "--no-warn-script-location"] + packages
...
def remove_cache_files(path_dir, remove_list):
    ...
    """
    # キャッシュフォルダを再帰的に検索
    dirs_to_remove = [path for path in glob(join(path_dir, "**", "*"), recursive=True) if (isdir(path) and basename(path) in remove_list)]
    # キャッシュフォルダを削除
...
def create_install_txt(path_out: str, version: str):
    ...
    """
    s = "\n".join(["type=editplugin", f"folder=ENUNU-{version}", f"contentsdir=ENUNU-{version}", "description=NNSVSモデルに歌ってもらうUTAUプラグイン"])
    with open(path_out, "w", encoding="cp932") as f:
        f.write(s)
