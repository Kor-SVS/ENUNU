...
def generate_full_align_lab(path_mono_align_lab_in, path_full_score_lab_in, path_full_align_lab_out):
    """
    タイミング補正済みのモノラベルと、
    タイミング補正前のフルラベルから、
    タイミング補正済みのフルラベルを作る。
...
def generate_mono_score_lab(path_full_score_lab_in, path_mono_align_lab_in, path_mono_score_lab_out):
    """
    タイミング補正済みのモノラベルと、
    タイミング補正前のフルラベルから、
    タイミング補正済みのフルラベルを作る。
...
if __name__ == "__main__":
    ...
    if len(argv) == 2:
        ...
        path_utauplugin = input("Input file path of TMP(plugin)\n>>> ").strip('"')
